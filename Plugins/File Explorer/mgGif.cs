...
namespace MG.GIF
{
        ...
        public class Decoder : IDisposable
    {
        ...
        public Image NextImage()
        {
            ...
            var block = Block.Start;
            while (block != Block.End)
            {
                block = (Block) ReadByte();
                switch (block)
                {
                    case Block.Image:
                    {
        ...
        void DecompressLZW()
        {
            ...
            fixed (byte* pData = Input)
            {
                ...
                {
                    var row = (Height - ImageTop - 1) *
                              Width; // start at end of array as we are reversing the row order
                    ...
                    while (true)
                    {
                        ...
                        if (previousCode != NoCode && numCodes != MaxCodes)
                        {
                            ...
                            if (pCodesEnd + codeLength + 1 >= pCodeBufferEnd)
                            {
                                ...
                                // realloc buffer
                                CodesLength *= 2;
                                CodesHandle = Marshal.ReAllocHGlobal(CodesHandle,
                                    (IntPtr) (CodesLength * sizeof(ushort)));
                            ...
                            *pCodesEnd++ = (ushort) (codeLength + 1);
                            ...
                            Buffer.MemoryCopy(pCodePos, pCodesEnd, codeLength * sizeof(ushort),
